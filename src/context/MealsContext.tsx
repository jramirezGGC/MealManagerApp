...
export function MealsProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchSavedMeals = async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        console.log("No saved meals document found");
        setSavedMeals([]);
      }
    } catch (err) {
  ...
  const loadMeals = async () => {
    await fetchMeals("fridge1");
    await fetchMeals("freezer1");
  };
  ...
  const refreshMeals = async () => {
    ...
    try {
      ...
      await loadMeals(); // Fetch current meals
      await fetchSavedMeals(); // Fetch saved meals
    } catch (error) {
